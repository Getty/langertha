...
`t/91_response_usage.t`; the seam is what is off. Tracked as karr k130.
...
- **A lift on `chat_response` does not reach the streaming route.** All three homes above sit on
  the non-streaming path; `Langertha::Stream::Chunk` has no `thinking` field, so a streamed call
  on the same engine and prompt reads `undef` (karr k129). Same class as ADR 0011's sync/async
  `ttft_seconds` gap.
...
- **karr k130** — move the `cached_tokens` wire spellings out of `Response::BUILDARGS` and onto
  `Langertha::Usage`, so home 1 holds for the cache read-back the way it already does for the
  token counts.
- **karr k129** — the streamed-thinking gap. A decision on `Stream::Chunk.thinking` will
  determine whether the three homes above need a streaming counterpart or whether the
  aggregation layer normalizes once.
